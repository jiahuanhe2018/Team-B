...
func makeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	...
	priv, _, err := crypto.GenerateKeyPairWithReader(crypto.RSA, 2048, r) // 生成公钥和私钥
	if err != nil {
		return nil, err
	}

	// 开始构建网络地址部分，这样其他节点就可以连接起来
	opts := []libp2p.Option{
	...
	return basicHost, nil  // 返回主机地址给调用函数
}
					...
					OUTER:
						...
						for _, block := range temp {
							if block.Validator == lotteryWinner {
								mutex.Lock()			 
								BlocksTemp.Blocks = append(Blockchain, block)
								mutex.Unlock()
...
func main() {
	...
	listenF := flag.Int("l", 0, "wait for incoming connections") // 是当前节点的监听主机地址，这样其它节点就可以连接进来，记住，每个节点都有两个身份：主机和客户端， 毕竟P2P不是白叫的
	target := flag.String("d", "", "target peer to dial")  // 指明当前节点要连接到的主机地址
	secio := flag.Bool("secio", false, "enable secio") // 是用来加密数据流的
	seed := flag.Int64("seed", 0, "set random seed for id generation") // 是随机数种子，用来创建主机地址时使用
	flag.Parse()

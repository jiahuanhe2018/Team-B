...
func main() {
	...
	typeDefine  := flag.String("t", "", "mode[ \"null\" or \"pow\"or \"pos\"]")
	...
	if *command == "chain" {
		runblockchain(listenF, target, typeDefine, seed, secio, suffix, initAccounts)
...
func runblockchain(listenF *int, target *string, typeDefine *string,seed *int64, secio *bool, suffix *string, initAccounts *string){
	t := time.Now()
	...
	genesisBlock = blockchain.Block{0, t.String(), 0, blockchain.CalculateHash(genesisBlock), "", 100,nil, defaultAccounts, 1, "", ""}
	...
	if "null" == *typeDefine {
		log.Println("null1111111111111111111")
		blockchain.ConsensusType = "null"

	}else if "pow" == *typeDefine {
		log.Println("pow")
		blockchain.ConsensusType = "pow"

	}else if "pos" == *typeDefine {
		log.Println("pos")
		blockchain.ConsensusType = "pos"
	...
	if *target == "" {
		...
		if "null" == blockchain.ConsensusType{
			// now do nothing
		}else if "pow" == blockchain.ConsensusType{
			go blockchain.WriteDataByPow(rw)
			go blockchain.ReadDataByPow(rw)
		}else if "pos" == blockchain.ConsensusType{
			go blockchain.ReadDataByPos(rw)
			go blockchain.WriteDataByPos(rw)
		}
